...
class C_Apriori extends Controller
{
    ...
    public function prosesAnalisaApriori(Request $request)
    {
        ...
        $nomorFaktur = M_Penjualan::distinct('no_faktur')->count('no_faktur');
        ...
        foreach($dataProduk as $produk){
            ...
            $nSupport = ($totalTransaksi / $nomorFaktur) * 100;
            $supp = new M_Support();
        ...
        foreach($nilaiKombinasi as $nk){
            ...
            $totalA = M_Penjualan::where('kd_barang', $kdBarangA)->count();
            $support = ($fnTransaksi / $totalA) * 100;
    ...
    public function hasilAnalisa(Request $request, $kdPengujian)
    {
        ...
        // $totalProduk = M_Produk::count();
        // M_Penjualan::distinct('no_faktur')->count('no_faktur');
        // dd($dataSupportProduk);
        $dr = [
            'dataSupport' => $dataSupportProduk,
            'totalProduk' => M_Penjualan::distinct('no_faktur')->count('no_faktur'),
    ...
    public function cetakAnalisa(Request $request, $kdPengujian)
    {
        try {
            ...
            $pdf = PDF::loadview('main.apriori.hello', $dr);
            return $pdf->stream();
        } catch (\Exception $e) {
